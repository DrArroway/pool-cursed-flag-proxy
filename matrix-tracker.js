// Wrap execution cleanly so DOM elements are guaranteed to exist
window.addEventListener('DOMContentLoaded', () => {
    ...
    function initArchiveCatalogUI() {
        ...
        // Safety call to set initial interface alignments
    ...
    function handleSourceViewToggle(val) {
        ...
        if (val === 'archive-browse') {
            ...
            document.getElementById('valSource').className = "badge bg-danger text-white";
            updateWebcamData();
        } else {
            if (archivePicker) archivePicker.style.display = 'none';
            if (youtubeContainer) youtubeContainer.style.display = 'none';
    ...
    function loadCustomArchiveTarget() {
        ...
        // Try standard filename structure
        ...
        img.onerror = function() {
            // Alternative filename lookup structures (root level fallbacks)
            const alternatives = [
    ...
    document.getElementById('streamUrlInput').addEventListener('change', (e) => {
        ...
        function drawFlag() {
            ...
            // --- FIXED CALIBRATION VISUALIZATION AS SPECT RATIO ---
            // If debug is active, draw the entire 16:9 background image across the WHOLE canvas at 75% alpha
            ...
            // Draw behind-the-scenes grid tiles if debug is off
            if (!config.debug) {
            ...
            starCoordinates.forEach((star) => {
                if (!config.debug) {
                    ...
                    // In debug mode, draw small pixel tracking markers inside the canton
                    ctx.fillStyle = star.color;
            ...
            // Draw Full-Scale Calibration Overlay Matching the Image Context Aspect Ratio!
            if (config.debug) {
                ctx.save();
        ...
        // Kickstart logic pipelines smoothly
        syncSlidersToConfig();
